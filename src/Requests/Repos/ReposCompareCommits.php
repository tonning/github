 ...
 *
 * Compares two commits against one another. You can compare branches in the same repository, or you
 * can compare branches that exist in different repositories within the same repository network,
 * including fork branches. For more information about how to view a repository's network, see
 ...
 * whereas the API returns commits in chronological order. You can pass the appropriate [media
 ...
 *
 * The API response includes details about the files that were
 ...
 * `renamed` status have a `previous_filename` field showing the previous filename of the file, and
 * files with a `modified` status have a `patch` field showing the changes made to the file.
 *
 * When
 * calling this endpoint without any paging parameter (`per_page` or `page`), the returned list is
 * limited to 250 commits, and the last commit in the list is the most recent of the entire
 * comparison.
 *
 * **Working with large comparisons**
 *
 * To process a response with a large number of
 * commits, use a query parameter (`per_page` or `page`) to paginate the results. When using
 * pagination:
 *
 * - The list of changed files is only shown on the first page of results, but it includes
 * all changed files for the entire comparison.
 * - The results are returned in chronological order, but
 * the last commit in the returned list may not be the most recent one in the entire set if there are
 * more pages of results.
 *
 * For more information on working with pagination, see "[Using pagination in
 * the REST API](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api)."
 *
 * **Signature
 * verification object**
 *
 ...
 *
 * These are the possible values for `reason` in the `verification`
 * object:
 *
 * | Value | Description |
 * | ----- | ----------- |
 ...
 * | `unverified_email` | The
 * `committer` email address in the commit was associated with a user, but the email address is not
